...
async def check_force_join_progressive(user_id: int, context: ContextTypes.DEFAULT_TYPE, user_name: str = "User") -> dict:
    """🌟 PREMIUM: Progressive force join checker"""
    force_channels = get_force_join_channels()
    ...
    for channel in force_channels:
        ...
        try:
            if is_private:
                pending = pending_requests_col.find_one({'user_id': user_id, 'channel_id': channel_id})
                if not pending:
                    not_joined.append(channel)
            else:
                try:
...
async def verify_progressive(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int, user_name: str = "User") -> dict:
    """🌟 PREMIUM: Progressive verification"""
    force_channels = get_force_join_channels()
    ...
    for channel in force_channels:
        ...
        verified_key = f"verified_{channel_id}"
        if context.user_data.get(verified_key):
            continue
        ...
        try:
            if is_private:
                ...
                if pending:
                    ...
                    context.user_data[verified_key] = True
                else:
    ...
    return {'all_done': False, 'message': message, 'keyboard': InlineKeyboardMarkup(buttons)}
