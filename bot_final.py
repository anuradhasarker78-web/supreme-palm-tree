...
async def handle_video_request(update: Update, context: ContextTypes.DEFAULT_TYPE, video_id: str):
    ...
    for channel in force_channels:
        try:
            ...
            # ✅ FIXED: Properly handle all membership statuses including PENDING (join request)
            # Telegram statuses: member, administrator, creator, restricted, left, kicked, PENDING
            
            # Log detailed status for debugging
            logger.info(f"🔍 Checking user {user.id} in {channel['username']}: status={member.status}")
            
            # Allow access if: member, admin, creator, OR join request pending
            if member.status in ['member', 'administrator', 'creator', 'restricted']:
                # User is a member or has sent join request
                logger.info(f"✅ User {user.id} HAS ACCESS to {channel['username']}: status={member.status}")
                # Continue to next channel
            elif member.status in ['left', 'kicked']:
                # User left or was kicked - block access
    ...
    if data.startswith("verify_"):
        ...
        for channel in force_channels:
            try:
                ...
                # Allow if: member, admin, creator, OR restricted (join request sent)
                if member.status in ['member', 'administrator', 'creator', 'restricted']:
                    logger.info(f"✅ Verify: User {user.id} HAS ACCESS to {channel['username']}: status={member.status}")
                    # Continue to next channel
                elif member.status in ['left', 'kicked']:
                    not_joined.append(channel)
